...
//     }
    ...
    public function export_sales_merge_all(){
        ...
        if(!empty($participant) && $participant!='null'){
            ...
            $sql.= " res_short_name IN($imp) AND ";
        } if(!empty($from) && !empty($from) && $from!='null' && $to != 'null'){
            $sql.= " ((res_billing_from BETWEEN '$from' AND '$to') OR (res_billing_to BETWEEN '$from' AND '$to')) AND ";
        } if($original!='null' && isset($original)){
             $sql.= "res_original_copy = '$original' AND "; 
        } if($scanned!='null'  && isset($scanned)){
             $sql.= "res_scanned_copy = '$scanned' AND "; 
        }
        ...
        foreach($this->super_model->custom_query("SELECT * FROM reserve_sales_transaction_head sth INNER JOIN reserve_sales_transaction_details std ON sth.reserve_sales_id = std.reserve_sales_id WHERE $qu ORDER BY res_billing_from ASC, res_reference_number ASC") AS $sth){
            $participant_name=$this->super_model->select_column_where("reserve_participant","res_participant_name","res_billing_id",$sth->res_billing_id);
            $short_name=$this->super_model->select_column_where("reserve_sales_transaction_details", "res_short_name", "reserve_sales_detail_id", $sth->reserve_sales_detail_id);
            $or_no=$this->super_model->select_column_custom_where("collection_reserve_details","series_number","reference_no='$sth->res_reference_number' AND settlement_id='$short_name'");
            if(!empty($sth->res_company_name) && date('Y',strtotime($sth->res_create_date))==date('Y')){
                    $comp_name=$sth->res_company_name;
                }else{
                    $comp_name=$participant_name;
                }
            $zero_rated=$sth->res_zero_rated_sales+$sth->res_zero_rated_ecozones;
            $total=($sth->res_vatable_sales+$zero_rated+$sth->res_vat_on_sales)-$sth->res_ewt;
            //$total_sum[]=$total;

            $data['salesall'][]=array(
                'participant_name'=>$comp_name,
                'billing_id'=>$sth->res_billing_id,
                'reference_number'=>$sth->res_reference_number,
                'sales_detail_id'=>$sth->reserve_sales_detail_id,
                'billing_from'=>$sth->res_billing_from,
                'billing_to'=>$sth->res_billing_to,
                'vatable_sales'=>$sth->res_vatable_sales,
                'vat_on_sales'=>$sth->res_vat_on_sales,
                'ewt'=>$sth->res_ewt,
                'ewt_amount'=>$sth->res_ewt_amount,
                'original_copy'=>$sth->res_original_copy,
                'scanned_copy'=>$sth->res_scanned_copy,
                'zero_rated'=>$zero_rated,
                'total'=>$total,
                'or_no'=>$or_no,
                );
        ...
        $data['total_sum']=array_sum($total_sum);
        $this->load->view('reports/reserve_sales_all',$data);
        $this->load->view('template/footer');
    }

    public function export_reserve_sales_all(){
        $participant=$this->uri->segment(3);
        $from=$this->uri->segment(4);
        $to=$this->uri->segment(5);
        $objPHPExcel = new Spreadsheet();
        $exportfilename="Reserve Sales Wesm All Transactions.xlsx";
        $sql='';
        ...
        foreach($this->super_model->custom_query("SELECT * FROM reserve_sales_transaction_head sth INNER JOIN reserve_sales_transaction_details std ON sth.reserve_sales_id = std.reserve_sales_id INNER JOIN reserve_participant p ON p.res_settlement_id = std.res_short_name WHERE res_participant_name != '' AND $qu GROUP BY res_tin ORDER BY res_settlement_id ASC") AS $head){
            ...
            foreach($this->super_model->custom_query("SELECT * FROM reserve_sales_transaction_head sth INNER JOIN reserve_sales_transaction_details std ON sth.reserve_sales_id = std.reserve_sales_id  INNER JOIN reserve_participant p ON p.res_billing_id = std.res_billing_id WHERE res_tin='$head->res_tin' AND res_participant_name != '' AND $qu ORDER BY res_billing_from ASC, res_reference_number ASC, p.res_billing_id ASC") AS $sth){
                $or_no=$this->super_model->select_column_custom_where("collection_reserve_details","series_number","reference_no='$sth->res_reference_number' AND settlement_id='$sth->res_short_name'");
                if(!empty($sth->res_company_name) && date('Y',strtotime($sth->res_create_date))==date('Y')){
                        $comp_name=$sth->res_company_name;
                    }else{
                        $comp_name=$sth->res_participant_name;
                    }
                $billing_date = date("M. d, Y",strtotime($sth->res_billing_from))." - ".date("M. d, Y",strtotime($sth->res_billing_to));
                $tin=$this->super_model->select_column_where("reserve_participant","res_tin","res_billing_id",$sth->res_billing_id);
                $salesall[]=array(
                    'billing_date'=>$billing_date,
                    'participant_name'=>$comp_name,
                    'billing_id'=>$sth->res_billing_id,
                    ...
                    'ewt'=>$sth->res_ewt,
                    'ewt_amount'=>$sth->res_ewt_amount,
                    'short_name'=>$sth->res_short_name,
                    'original_copy'=>$sth->res_original_copy,
                    'scanned_copy'=>$sth->res_scanned_copy,
                    'zero_rated_sales'=>$sth->res_zero_rated_sales,
                    'zero_rated_ecozones'=>$sth->res_zero_rated_ecozones,
                    'tin'=>$tin,
                    'or_no'=>$or_no,
                );
            }
        ...
        header('Content-Type: application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
        header('Content-Disposition: attachment;filename="Reserve Sales Wesm All Transactions.xlsx"');
        header('Cache-Control: max-age=0');
        $objWriter = io_factory::createWriter($objPHPExcel, 'Xlsx');
        $objWriter->save('php://output');
    }
    ...
    public function purchases_all_adjustment(){
        ...
        $sheetno=0;
            ...
            $objPHPExcel->setActiveSheetIndex($sheetno)->setCellValue('H1', "Zip Code");
            $objPHPExcel->setActiveSheetIndex($sheetno)->setCellValue('I1', "Vatables Purchases");
            $objPHPExcel->setActiveSheetIndex($sheetno)->setCellValue('J1', "Zero-rated Purchases");
            $objPHPExcel->setActiveSheetIndex($sheetno)->setCellValue('K1', "Zero-rated Ecozones Purchases");
            $objPHPExcel->setActiveSheetIndex($sheetno)->setCellValue('L1', "Vat On Purchases");
            $objPHPExcel->setActiveSheetIndex($sheetno)->setCellValue('M1', "EWT");
            $objPHPExcel->setActiveSheetIndex($sheetno)->setCellValue('N1', "Total");
            $objPHPExcel->setActiveSheetIndex($sheetno)->setCellValue('O1', "OR Number");
            $objPHPExcel->setActiveSheetIndex($sheetno)->setCellValue('P1', "Total Amount");
            $objPHPExcel->setActiveSheetIndex($sheetno)->setCellValue('Q1', "Original Copy");
            $objPHPExcel->setActiveSheetIndex($sheetno)->setCellValue('R1', "Scanned Copy");
            ...
            $zip_code=$this->super_model->select_column_where("participant","zip_code","billing_id",$pah->billing_id);
            ...
            if($value['tin']==$pah->tin){
               // echo $value['tin']." / ".$pah->tin."<br>";
                ...
                $objPHPExcel->setActiveSheetIndex($sheetno)->setCellValue('H'.$num, $value['zip_code']);
                $objPHPExcel->setActiveSheetIndex($sheetno)->setCellValue('I'.$num, "-".$value['vatables_purchases']);
                $objPHPExcel->setActiveSheetIndex($sheetno)->setCellValue('J'.$num, "-".$value['zero_rated_purchases']);
                $objPHPExcel->setActiveSheetIndex($sheetno)->setCellValue('K'.$num, "-".$value['zero_rated_ecozones']);
                $objPHPExcel->setActiveSheetIndex($sheetno)->setCellValue('L'.$num, "-".$value['vat_on_purchases']);
                $objPHPExcel->setActiveSheetIndex($sheetno)->setCellValue('M'.$num, $value['ewt']);
                $objPHPExcel->setActiveSheetIndex($sheetno)->setCellValue('N'.$num, "-".$total);
                $objPHPExcel->setActiveSheetIndex($sheetno)->setCellValue('O'.$num, $value['or_no']);
                $objPHPExcel->setActiveSheetIndex($sheetno)->setCellValue('P'.$num, "-".$value['total_update']);
                if($value['original_copy']==1){
                ...
                if($value['scanned_copy']==1){
                    $objPHPExcel->setActiveSheetIndex($sheetno)->setCellValue('R'.$num, "Yes");
                }else if($value['scanned_copy']==0){
                    $objPHPExcel->setActiveSheetIndex($sheetno)->setCellValue('R'.$num, "No");
                }else{
                    $objPHPExcel->setActiveSheetIndex($sheetno)->setCellValue('R'.$num, "");
                }
                ...
                $objPHPExcel->getActiveSheet()->getStyle('A1:R1')->getFill()->setFillType(fill::FILL_SOLID)->getStartColor()->setARGB('1c4966');
                ...
                $objPHPExcel->getActiveSheet()->getStyle('H'.$num.":R".$num)->getAlignment()->setHorizontal(alignment::HORIZONTAL_CENTER);
                $objPHPExcel->getActiveSheet()->getStyle('I'.$num.":N".$num)->getNumberFormat()->setFormatCode(numberformat::FORMAT_NUMBER_COMMA_SEPARATED1);
                $objPHPExcel->getActiveSheet()->getStyle('P'.$num)->getNumberFormat()->setFormatCode(numberformat::FORMAT_NUMBER_COMMA_SEPARATED1);
                $objPHPExcel->getActiveSheet()->getStyle('A1:R1')->getFont()->setBold(true);
                $objPHPExcel->getActiveSheet()->getStyle('A1:R1')->getAlignment()->setHorizontal(alignment::HORIZONTAL_CENTER);
                $num++;
            }
        }
                $a = $num;
                    ...
                    $objPHPExcel->getActiveSheet()->getStyle('P'.$a)->getAlignment()->setHorizontal(alignment::HORIZONTAL_CENTER);
                    $objPHPExcel->getActiveSheet()->getStyle("I".$a.':M'.$a)->getNumberFormat()->setFormatCode(numberformat::FORMAT_NUMBER_COMMA_SEPARATED1);
                    $objPHPExcel->getActiveSheet()->getStyle("P".$a)->getNumberFormat()->setFormatCode(numberformat::FORMAT_NUMBER_COMMA_SEPARATED1);
                    //$objPHPExcel->getActiveSheet()->setCellValue('D'.$a, "TOTAL: ");
                    $objPHPExcel->getActiveSheet()->setCellValue('I'.$a, "-".array_sum($total_vatables));
                    $objPHPExcel->getActiveSheet()->setCellValue('J'.$a, "-".array_sum($total_zerorated_purchases));
                    $objPHPExcel->getActiveSheet()->setCellValue('K'.$a, "-".array_sum($total_zerorated_ecozones));
                    $objPHPExcel->getActiveSheet()->setCellValue('L'.$a, "-".array_sum($total_vat));
                    $objPHPExcel->getActiveSheet()->setCellValue('M'.$a, array_sum($total_ewt));
                    $objPHPExcel->getActiveSheet()->setCellValue('N'.$a, "-".array_sum($overall_total));
                    $objPHPExcel->getActiveSheet()->setCellValue('P'.$a, "-".array_sum($total_update_amount));
                $num--;
            $sheetno++;
        }
        header('Content-Type: application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
        ...
        $sheetno=0;
            ...
            $objPHPExcel->setActiveSheetIndex($sheetno)->setCellValue('I1', "Address");
            $objPHPExcel->setActiveSheetIndex($sheetno)->setCellValue('J1', "Zip Code");
            $objPHPExcel->setActiveSheetIndex($sheetno)->setCellValue('K1', "Vatable Sales");
            $objPHPExcel->setActiveSheetIndex($sheetno)->setCellValue('L1', "Zero Rated EcoZones Sales");
            $objPHPExcel->setActiveSheetIndex($sheetno)->setCellValue('M1', "Vat On Sales");
            $objPHPExcel->setActiveSheetIndex($sheetno)->setCellValue('N1', "EWT");
            $objPHPExcel->setActiveSheetIndex($sheetno)->setCellValue('O1', "Total");
            ...
            $itemno=1;
                ...
                // //if($value['tin']==$pah->tin){
                    ...
                    $objPHPExcel->setActiveSheetIndex($sheetno)->setCellValue('I'.$num, $address);
                    $objPHPExcel->setActiveSheetIndex($sheetno)->setCellValue('J'.$num, $zip_code);
                    $objPHPExcel->setActiveSheetIndex($sheetno)->setCellValue('K'.$num, "-".$pah->vatables_purchases);
                    $objPHPExcel->setActiveSheetIndex($sheetno)->setCellValue('L'.$num, "-".$pah->zero_rated_ecozones);
                    $objPHPExcel->setActiveSheetIndex($sheetno)->setCellValue('M'.$num, "-".$pah->vat_on_purchases);
                    $objPHPExcel->setActiveSheetIndex($sheetno)->setCellValue('N'.$num, $pah->ewt);
                    $objPHPExcel->setActiveSheetIndex($sheetno)->setCellValue('O'.$num, "-".$total);
                // }

                    $objPHPExcel->getActiveSheet()->getStyle('A1:O1')->getFill()->setFillType(fill::FILL_SOLID)->getStartColor()->setARGB('1c4966');
                    $objPHPExcel->getActiveSheet()->getStyle('A1:O1')->getFont()->getColor()->setRGB ('FFFFFF');
                    $objPHPExcel->getActiveSheet()->getStyle('G'.$num.":J".$num)->getFill()->setFillType(fill::FILL_SOLID)->getStartColor()->setARGB('BFD7ED');
                    ...
                    $objPHPExcel->getActiveSheet()->getStyle('K'.$num.":O".$num)->getAlignment()->setHorizontal(alignment::HORIZONTAL_CENTER);
                    $objPHPExcel->getActiveSheet()->getStyle('K'.$num.":O".$num)->getNumberFormat()->setFormatCode(numberformat::FORMAT_NUMBER_COMMA_SEPARATED1);
                ...
                $a = $num;
                    $objPHPExcel->getActiveSheet()->getStyle('K'.$a.":O".$a)->getFont()->setBold(true);
                    $objPHPExcel->getActiveSheet()->getStyle('K'.$a.":O".$a)->getFont()->setItalic(true);
                    $objPHPExcel->getActiveSheet()->getStyle('K'.$a.":O".$a)->getAlignment()->setHorizontal(alignment::HORIZONTAL_CENTER);
                    $objPHPExcel->getActiveSheet()->getStyle('K'.$a.":O".$a)->getNumberFormat()->setFormatCode(numberformat::FORMAT_NUMBER_COMMA_SEPARATED1);
                    $objPHPExcel->getActiveSheet()->getStyle('K'.$a.":O".$a)->getFill()->setFillType(fill::FILL_SOLID)->getStartColor()->setARGB('FFFF00');
                    $objPHPExcel->getActiveSheet()->setCellValue('K'.$a, "-".array_sum($total_vatables_purchases));
                    $objPHPExcel->getActiveSheet()->setCellValue('L'.$a, "-".array_sum($total_zero_rated_ecozones));
                    $objPHPExcel->getActiveSheet()->setCellValue('M'.$a, "-".array_sum($total_vat_on_purchases));
                    $objPHPExcel->getActiveSheet()->setCellValue('N'.$a, array_sum($total_ewt));
                    $objPHPExcel->getActiveSheet()->setCellValue('O'.$a, "-".array_sum($overall_total));
                $num--;
    ...
    public function sales_all_adjustment(){
        ...
        $sheetno=0;
            ...
            $objPHPExcel->setActiveSheetIndex($sheetno)->setCellValue('G1', "Address");
            $objPHPExcel->setActiveSheetIndex($sheetno)->setCellValue('H1', "Zip Code");
            $objPHPExcel->setActiveSheetIndex($sheetno)->setCellValue('I1', "Vatables Sales");
            ...
            $objPHPExcel->setActiveSheetIndex($sheetno)->setCellValue('L1', "EWT Sales");
            $objPHPExcel->setActiveSheetIndex($sheetno)->setCellValue('M1', "Total");
            $objPHPExcel->setActiveSheetIndex($sheetno)->setCellValue('N1', "EWT Amount");
            $objPHPExcel->setActiveSheetIndex($sheetno)->setCellValue('O1', "Original Copy");
            $objPHPExcel->setActiveSheetIndex($sheetno)->setCellValue('P1', "Scanned Copy");
            $objPHPExcel->setActiveSheetIndex($sheetno)->setCellValue('Q1', "OR Number");
            ...
            $address=$this->super_model->select_column_where("participant","registered_address","billing_id",$sah->billing_id);
            $zip_code=$this->super_model->select_column_where("participant","zip_code","billing_id",$sah->billing_id);
            ...
            $salesalladjustment[]=array(
                ...
                $objPHPExcel->setActiveSheetIndex($sheetno)->setCellValue('G'.$num, $value['address']);
                $objPHPExcel->setActiveSheetIndex($sheetno)->setCellValue('H'.$num, $value['zip_code']);
                $objPHPExcel->setActiveSheetIndex($sheetno)->setCellValue('I'.$num, $value['vatable_sales']);
                $objPHPExcel->setActiveSheetIndex($sheetno)->setCellValue('J'.$num, $zero_rated);
                $objPHPExcel->setActiveSheetIndex($sheetno)->setCellValue('K'.$num, $value['vat_on_sales']);
                ...
                $objPHPExcel->setActiveSheetIndex($sheetno)->setCellValue('N'.$num, $value['ewt_amount']);
                if($value['original_copy']==1){
                    $objPHPExcel->setActiveSheetIndex($sheetno)->setCellValue('O'.$num, "Yes");
                }else if($value['original_copy']==0){
                    $objPHPExcel->setActiveSheetIndex($sheetno)->setCellValue('O'.$num, "No");
                }else{
                    $objPHPExcel->setActiveSheetIndex($sheetno)->setCellValue('O'.$num, "");
                }
                if($value['scanned_copy']==1){
                ...
                $objPHPExcel->setActiveSheetIndex($sheetno)->setCellValue('Q'.$num, $value['or_no']);
               ...
               // echo $previousKey."-".$nextKey."-".$value['due_date']." - ".$value['short_name']." - ".$value['billing_date']."<br>";
                ...
                $a = $num;
                    //$objPHPExcel->getActiveSheet()->getStyle('D'.$a)->getFont()->setBold(true);
                    $objPHPExcel->getActiveSheet()->getStyle('K'.$a.":N".$a)->getAlignment()->setHorizontal(alignment::HORIZONTAL_CENTER);
                    $objPHPExcel->getActiveSheet()->getStyle("K".$a.':N'.$a)->getNumberFormat()->setFormatCode(numberformat::FORMAT_NUMBER_COMMA_SEPARATED1);
                    //$objPHPExcel->getActiveSheet()->setCellValue('D'.$a, "TOTAL: ");
                    //$objPHPExcel->getActiveSheet()->setCellValue('E'.$a, array_sum($total_vatables));
                    //$objPHPExcel->getActiveSheet()->setCellValue('F'.$a, array_sum($total_zero_rated));
                    //$objPHPExcel->getActiveSheet()->setCellValue('G'.$a, array_sum($total_vat));
                    ...
                    //$objPHPExcel->getActiveSheet()->setCellValue('I'.$a, array_sum($overall_total));
                    $objPHPExcel->getActiveSheet()->setCellValue('N'.$a, array_sum($total_ewt_amount));
                $num--;
            $sheetno++;
        }
        header('Content-Type: application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
        ...
        $sheetno=0;
            ...
            $itemno=1;
                ...
                // //if($value['tin']==$sah->tin){
                    ...
                    $objPHPExcel->setActiveSheetIndex($sheetno)->setCellValue('I'.$num, $address);
                    $objPHPExcel->setActiveSheetIndex($sheetno)->setCellValue('J'.$num, $zip_code);
                    $objPHPExcel->setActiveSheetIndex($sheetno)->setCellValue('K'.$num, $sah->vatable_sales);
                    $objPHPExcel->setActiveSheetIndex($sheetno)->setCellValue('L'.$num, $sah->zero_rated_ecozones);
                    $objPHPExcel->setActiveSheetIndex($sheetno)->setCellValue('M'.$num, $sah->vat_on_sales);
                    $objPHPExcel->setActiveSheetIndex($sheetno)->setCellValue('N'.$num, "-".$sah->ewt);
                    $objPHPExcel->setActiveSheetIndex($sheetno)->setCellValue('O'.$num, $total);
                // }
                ...
                $a = $num;
                    ...
                    $objPHPExcel->getActiveSheet()->getStyle('K'.$a.":O".$a)->getNumberFormat()->setFormatCode(numberformat::FORMAT_NUMBER_COMMA_SEPARATED1);
                    $objPHPExcel->getActiveSheet()->getStyle('K'.$a.":O".$a)->getFill()->setFillType(fill::FILL_SOLID)->getStartColor()->setARGB('FFFF00');
                    $objPHPExcel->getActiveSheet()->setCellValue('K'.$a, array_sum($total_vatable_sales));
                    $objPHPExcel->getActiveSheet()->setCellValue('L'.$a, array_sum($total_zero_rated_ecozones));
                    $objPHPExcel->getActiveSheet()->setCellValue('M'.$a, array_sum($total_vat_on_sales));
                    $objPHPExcel->getActiveSheet()->setCellValue('N'.$a, "-".array_sum($total_ewt));
                    $objPHPExcel->getActiveSheet()->setCellValue('O'.$a, array_sum($overall_total));
                $num--;
        ...
        //$data=array();
            ...
            $row=7;
                foreach($this->super_model->custom_query("SELECT * FROM collection_head ch INNER JOIN collection_details cd ON ch.collection_id = cd.collection_id WHERE $qu GROUP BY series_number ORDER BY series_number ASC") AS $col){
                    ...
                    $sum_amount= $this->super_model->select_sum_where("collection_details","amount","collection_id = '$col->collection_id' AND series_number = '$col->series_number'");
                    $sum_zero_rated= $this->super_model->select_sum_where("collection_details","zero_rated","collection_id = '$col->collection_id' AND series_number = '$col->series_number'");
                    $sum_zero_rated_ecozone= $this->super_model->select_sum_where("collection_details","zero_rated_ecozone","collection_id = '$col->collection_id' AND series_number = '$col->series_number'");
                    $sum_vat = $this->super_model->select_sum_where("collection_details","vat","collection_id = '$col->collection_id' AND series_number = '$col->series_number'");
                    $sum_ewt= $this->super_model->select_sum_where("collection_details","ewt","collection_id = '$col->collection_id' AND series_number = '$col->series_number'");
                    $overall_total=($sum_amount + $sum_zero_rated + $sum_zero_rated_ecozone + $sum_vat)-$sum_ewt;
    ...
    public function reserve_collection_report(){
        ...
        foreach($this->super_model->custom_query("SELECT * FROM collection_reserve_head ch INNER JOIN collection_reserve_details cd ON ch.res_collection_id = cd.res_collection_id WHERE $qu") AS $col){
            ...
            $data['collection'][]=array(
                "count_series"=>$count_series,
                "collection_details_id"=>$col->res_collection_details_id,
                "collection_id"=>$col->res_collection_id,
                "settlement_id"=>$col->settlement_id,
                "series_number"=>$col->series_number,
                "collection_date"=>$col->collection_date,
                "billing_remarks"=>$col->billing_remarks,
        ...
        $this->load->view('reports/reserve_collection_report',$data);
        $this->load->view('template/footer');
    }

    public function export_reserve_collection_report(){

        $date=$this->uri->segment(3);
        $ref_no=$this->uri->segment(4);
        $stl_id=$this->uri->segment(5);
        $objPHPExcel = new Spreadsheet();
        $exportfilename="Reserve Collection Reports.xlsx";
        $sql='';
        if($date!='null'){
             $sql.= " collection_date = '$date' AND "; 
        } if($ref_no!='null'){
        ...
        header('Content-Type: application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
        header('Content-Disposition: attachment;filename="Reserve Collection Reports.xlsx"');
        header('Cache-Control: max-age=0');
        $objWriter = io_factory::createWriter($objPHPExcel, 'Xlsx');
        $objWriter->save('php://output');
        ...
        public function export_reserve_iemop(){

        $date=$this->uri->segment(3);
        $ref_no=$this->uri->segment(4);
        $stl_id=$this->uri->segment(5);
        $objPHPExcel = new Spreadsheet();
        $exportfilename="IEMOP Reserve Collection Reports.xlsx";
        $sql='';
        if($date!='null'){
             $sql.= " collection_date = '$date' AND "; 
        } if($ref_no!='null'){
        ...
        //$data=array();
            ...
            $row=7;
                foreach($this->super_model->custom_query("SELECT * FROM collection_reserve_head ch INNER JOIN collection_reserve_details cd ON ch.res_collection_id = cd.res_collection_id WHERE $qu AND series_number != 0 GROUP BY series_number ORDER BY series_number ASC") AS $col){
                    ...
                    $zip = $this->super_model->select_column_where("reserve_participant","res_zip_code","res_settlement_id",$col->settlement_id);
        ...
        header('Content-Type: application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
        header('Content-Disposition: attachment;filename="IEMOP Reserve Collection Reports.xlsx"');
        header('Cache-Control: max-age=0');
        $objWriter = io_factory::createWriter($objPHPExcel, 'Xlsx');
        $objWriter->save('php://output');
    ...
    public function export_unpaid_invoices_salesadj(){
        ...
        if($year!='null' && !empty($year)){
            $sql.= " YEAR(res_due_date) = '$year' AND ";
        }
        
        if($due_date!='null' && !empty($due_date)){
            $sql.= " res_due_date = '$due_date' AND ";
        }
        ...
        $total_ewt_balance=array();
            ...
            if($count_collection == 0 && $total != 0){
                $data['unpaid_sales'][]=array(
                    "date"=>$ui->res_transaction_date,
                    "due_date"=>$ui->res_due_date,
                    "billing_from"=>$ui->res_billing_from,
                    "billing_to"=>$ui->res_billing_to,
                    "reference_number"=>$ui->res_reference_number,
                    "vatable_sales"=>$ui->res_vatable_sales,
                    "zero_rated_sales"=>$ui->res_zero_rated_ecozones,
                    "vat_on_sales"=>$ui->res_vat_on_sales,
                    "ewt"=>$ui->res_ewt,
                    "stl_id"=>$ui->res_short_name,
                    "billing_id"=>$ui->res_billing_id,
                    "invoice_no"=>$ui->res_serial_no,
                    "total"=>$total,
                    "days_lapsed"=>$days_lapsed,
                    );
                }
            }
        $this->load->view('reports/unpaid_invoices_reserve_sales',$data);
        $this->load->view('template/footer');
    }

    public function export_unpaid_invoices_reserve_sales(){
        $year=$this->uri->segment(3);
        $due_date=$this->uri->segment(4);
        $today = date('F j, Y');
        $objPHPExcel = new Spreadsheet();
        $exportfilename="Summary of Unpaid Invoices (Reserve Sales).xlsx";
        $sql='';
            ...
            foreach($this->super_model->custom_query("SELECT * FROM reserve_sales_transaction_details std INNER JOIN reserve_sales_transaction_head sth ON std.reserve_sales_id=sth.reserve_sales_id WHERE $qu ORDER BY res_short_name ASC") AS $ui){
                ...
                $total_vatable[] = $ui->res_vatable_sales;
                $total_zero_rated[] = $ui->res_zero_rated_ecozones;
                $total_vat[] = $ui->res_vat_on_sales;
                $total_overdue[] = $total;

                $objPHPExcel->setActiveSheetIndex(0)->setCellValue('A'.$num, $ui->res_transaction_date);
                $objPHPExcel->setActiveSheetIndex(0)->setCellValue('B'.$num, $ui->res_serial_no);
                $objPHPExcel->setActiveSheetIndex(0)->setCellValue('C'.$num, $ui->res_due_date);
                $objPHPExcel->setActiveSheetIndex(0)->setCellValue('D'.$num, $ui->res_reference_number);
                $objPHPExcel->setActiveSheetIndex(0)->setCellValue('G'.$num, $ui->res_short_name);
                $objPHPExcel->setActiveSheetIndex(0)->setCellValue('I'.$num, $ui->res_billing_id);
                $objPHPExcel->setActiveSheetIndex(0)->setCellValue('K'.$num, $ui->res_vatable_sales);
                $objPHPExcel->setActiveSheetIndex(0)->setCellValue('M'.$num, $ui->res_zero_rated_ecozones);
                $objPHPExcel->setActiveSheetIndex(0)->setCellValue('O'.$num, $ui->res_vat_on_sales);
                $objPHPExcel->setActiveSheetIndex(0)->setCellValue('Q'.$num, $total);
                $objPHPExcel->setActiveSheetIndex(0)->setCellValue('S'.$num, $overdue);
                    ...
                    $num--;
        header('Content-Type: application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
        header('Content-Disposition: attachment;filename="Summary of Unpaid Invoices (Reserve Sales).xlsx"');
        header('Cache-Control: max-age=0');
        $objWriter = io_factory::createWriter($objPHPExcel, 'Xlsx');
        $objWriter->save('php://output');
    }
    ...
    public function export_monthly_IEMOP_purchases(){
        ...
        $objPHPExcel->setActiveSheetIndex(0)->setCellValue('B4', "$due_date");
            ...
            $objPHPExcel->setActiveSheetIndex(0)->setCellValue('J6', "Zip Code");
            $objPHPExcel->setActiveSheetIndex(0)->setCellValue('K6', "Statement No (Seller)");
            $objPHPExcel->setActiveSheetIndex(0)->setCellValue('L6', "Vatable Purchases");
            $objPHPExcel->setActiveSheetIndex(0)->setCellValue('M6', "VAT on Purchases");
            $objPHPExcel->setActiveSheetIndex(0)->setCellValue('N6', "Zero Rated Ecozone");
            $objPHPExcel->setActiveSheetIndex(0)->setCellValue('O6', "EWT Purchases");
           
            $objPHPExcel->getActiveSheet()->getStyle("A6:O6")->applyFromArray($styleArray);

            $row=7;
            
                foreach($this->super_model->custom_query("SELECT * FROM purchase_transaction_head pth INNER JOIN purchase_transaction_details ptd ON pth.purchase_id = ptd.purchase_id WHERE $qu ORDER BY transaction_date ASC") AS $col){
                    ...
                    $zip = $this->super_model->select_column_where("participant","zip_code","settlement_id",$col->short_name);
                        ...
                        $objPHPExcel->setActiveSheetIndex(0)->setCellValue('J'.$row, $zip);
                        $objPHPExcel->setActiveSheetIndex(0)->setCellValue('K'.$row, $col->reference_number);
                        $objPHPExcel->setActiveSheetIndex(0)->setCellValue('L'.$row, $col->vatables_purchases);
                        $objPHPExcel->setActiveSheetIndex(0)->setCellValue('M'.$row, $col->vat_on_purchases);
                        $objPHPExcel->setActiveSheetIndex(0)->setCellValue('N'.$row, $col->zero_rated_ecozones);
                        $objPHPExcel->setActiveSheetIndex(0)->setCellValue('O'.$row, $col->ewt);
                    ...
                    $objPHPExcel->createSheet();
                    $objPHPExcel->setActiveSheetIndex(1);
                    $objPHPExcel->getActiveSheet(1)->setTitle('BIR SLSP - Vat on Purchases');
                    ...
                    $objPHPExcel->setActiveSheetIndex(1)->setCellValue('J6', "Zip Code");
                    $objPHPExcel->setActiveSheetIndex(1)->setCellValue('K6', "Statement No (Seller)");
                    $objPHPExcel->setActiveSheetIndex(1)->setCellValue('L6', "Vatable Purchases");
                    $objPHPExcel->setActiveSheetIndex(1)->setCellValue('M6', "VAT on Purchases");
                    $objPHPExcel->setActiveSheetIndex(1)->setCellValue('O6', "EWT Purchases");
                    $objPHPExcel->getActiveSheet(1)->getStyle("A6:O6")->applyFromArray($styleArray);
                    ...
                    $row=7;
                     foreach($this->super_model->custom_query("SELECT * FROM purchase_transaction_head pth INNER JOIN purchase_transaction_details ptd ON pth.purchase_id = ptd.purchase_id WHERE $qu ORDER BY transaction_date ASC") AS $col){
                        ...
                        $zip = $this->super_model->select_column_where("participant","zip_code","settlement_id",$col->short_name);
                            ...
                            $objPHPExcel->setActiveSheetIndex(1)->setCellValue('J'.$row, $zip);
                            $objPHPExcel->setActiveSheetIndex(1)->setCellValue('K'.$row, $col->reference_number);
                            $objPHPExcel->setActiveSheetIndex(1)->setCellValue('L'.$row, $col->vatables_purchases);
                            $objPHPExcel->setActiveSheetIndex(1)->setCellValue('M'.$row, $col->vat_on_purchases);
                            $objPHPExcel->setActiveSheetIndex(1)->setCellValue('O'.$row, $col->ewt);

                                $objPHPExcel->getActiveSheet(1)->getStyle('A'.$row.":O".$row)->applyFromArray($styleArray);
                                $objPHPExcel->getActiveSheet(1)->getStyle('G'.$row)->getAlignment()->setHorizontal(alignment::HORIZONTAL_CENTER);
                                $objPHPExcel->getActiveSheet(1)->getStyle('L'.$row.":O".$row)->getAlignment()->setHorizontal(alignment::HORIZONTAL_CENTER);
                                $objPHPExcel->getActiveSheet(1)->getStyle('L'.$row.":M".$row)->getNumberFormat()->setFormatCode(numberformat::FORMAT_NUMBER_COMMA_SEPARATED1);
                                $row++;
                            }

                    $objPHPExcel->getActiveSheet(1)->getStyle('A6:O6')->getAlignment()->setWrapText(true);
                    $objPHPExcel->getActiveSheet(1)->getStyle('A6:O6')->getFill()->setFillType(fill::FILL_SOLID)->getStartColor()->setARGB('1c4966');
                    $objPHPExcel->getActiveSheet(1)->getStyle('A6:O6')->getFont()->getColor()->setRGB ('FFFFFF');
                    $objPHPExcel->getActiveSheet(1)->getStyle('A6:O6')->getFont()->setBold(true);
                    $objPHPExcel->getActiveSheet(1)->getStyle('A6:O6')->getAlignment()->setHorizontal(alignment::HORIZONTAL_CENTER);
                    ...
                    foreach($this->super_model->custom_query("SELECT * FROM purchase_transaction_head pth INNER JOIN purchase_transaction_details ptd ON pth.purchase_id = ptd.purchase_id WHERE $qu ORDER BY transaction_date ASC") AS $col){
                        ...
                        $zip = $this->super_model->select_column_where("participant","zip_code","settlement_id",$col->short_name);
                            ...
                            $objPHPExcel->getActiveSheet(2)->getStyle('A'.$row.":L".$row)->applyFromArray($styleArray);
                            $objPHPExcel->getActiveSheet(2)->getStyle('G'.$row)->getAlignment()->setHorizontal(alignment::HORIZONTAL_CENTER);
                            $objPHPExcel->getActiveSheet(2)->getStyle('A'.$row.":L".$row)->getAlignment()->setHorizontal(alignment::HORIZONTAL_CENTER);
                            $objPHPExcel->getActiveSheet(2)->getStyle('L'.$row)->getNumberFormat()->setFormatCode(numberformat::FORMAT_NUMBER_COMMA_SEPARATED1);
                            $row++;
                        }
        ...
        header('Content-Type: application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
        header('Content-Disposition: attachment;filename="IEMOP Purchases_'.$fname.'.xlsx"');
        header('Cache-Control: max-age=0');
        $objWriter = io_factory::createWriter($objPHPExcel, 'Xlsx');
        $objWriter->save('php://output');
    }

    public function export_monthly_IEMOP_adjustment_purchases(){

        $due=$this->uri->segment(3);
        $objPHPExcel = new Spreadsheet();
        $exportfilename="Monthly IEMOP Collection Reports - Purchases Adjustment.xlsx";
        $sql='';
        if($due!='null'){
             $sql.= " due_date = '$due' AND "; 
        }
                     ...
                     foreach($this->super_model->custom_query("SELECT * FROM purchase_transaction_head pth INNER JOIN purchase_transaction_details ptd ON pth.purchase_id = ptd.purchase_id WHERE $qu ORDER BY transaction_date ASC") AS $col){
                        ...
                        $zip = $this->super_model->select_column_where("participant","zip_code","settlement_id",$col->short_name);
                            ...
                            $objPHPExcel->setActiveSheetIndex(1)->setCellValue('O'.$row, $col->ewt);
                                ...
                                $objPHPExcel->getActiveSheet(1)->getStyle('G'.$row)->getAlignment()->setHorizontal(alignment::HORIZONTAL_CENTER);
                                $objPHPExcel->getActiveSheet(1)->getStyle('L'.$row.":O".$row)->getAlignment()->setHorizontal(alignment::HORIZONTAL_CENTER);
                                $objPHPExcel->getActiveSheet(1)->getStyle('L'.$row.":O".$row)->getNumberFormat()->setFormatCode(numberformat::FORMAT_NUMBER_COMMA_SEPARATED1);
                                $row++;
                            }
        ...
        header('Content-Type: application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
        header('Content-Disposition: attachment;filename="IEMOP Purchases Adjustment_'.$fname.'.xlsx"');
        header('Cache-Control: max-age=0');
        $objWriter = io_factory::createWriter($objPHPExcel, 'Xlsx');
        $objWriter->save('php://output');
    ...
    public function collection_report_merge(){
        ...
        $this->load->view('reports/collection_report_merge',$data);
        $this->load->view('template/footer');
    }

    public function export_merge_collection_report(){
        $date=$this->uri->segment(3);
        $ref_no=$this->uri->segment(4);
        $stl_id=$this->uri->segment(5);
        $objPHPExcel = new Spreadsheet();
        $exportfilename="Collection Reports (Merge).xlsx";
        $sql='';
        if($date!='null'){
             $sql.= " collection_date = '$date' AND "; 
        } if($ref_no!='null'){
       ...
       // echo  $qu . "<br>";
            $row=7;
            $row_final=7;
         foreach($this->super_model->custom_query("SELECT DISTINCT  reference_no, settlement_id, collection_date, series_number FROM merge_collection_head ch INNER JOIN merge_collection_details cd ON ch.merge_collection_id = cd.merge_collection_id WHERE $qu ORDER BY series_number ASC") AS $q){

              $x=1;
              $final=1;
                $count = $this->super_model->count_custom("SELECT * FROM merge_collection_head ch INNER JOIN merge_collection_details cd ON ch.merge_collection_id = cd.merge_collection_id WHERE $qu AND reference_no = '$q->reference_no' AND settlement_id = '$q->settlement_id' AND collection_date = '$q->collection_date' AND series_number='$q->series_number'");
                foreach($this->super_model->custom_query("SELECT * FROM merge_collection_head ch INNER JOIN merge_collection_details cd ON ch.merge_collection_id = cd.merge_collection_id WHERE $qu AND reference_no = '$q->reference_no' 
                    AND settlement_id = '$q->settlement_id' AND collection_date = '$q->collection_date' AND series_number='$q->series_number' ORDER BY series_number ASC") AS $col){
        ...
        header('Content-Type: application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
        header('Content-Disposition: attachment;filename="Collection Reports (Merge).xlsx"');
        header('Cache-Control: max-age=0');
        $objWriter = io_factory::createWriter($objPHPExcel, 'Xlsx');
        $objWriter->save('php://output');
    }

    public function export_merge_iemop(){
        $date=$this->uri->segment(3);
        $ref_no=$this->uri->segment(4);
        $stl_id=$this->uri->segment(5);
        $objPHPExcel = new Spreadsheet();
        $exportfilename="IEMOP Collection Reports (Merge).xlsx";
        $sql='';
        if($date!='null'){
             $sql.= " collection_date = '$date' AND "; 
        } if($ref_no!='null'){
        ...
        //$data=array();
            ...
            $row=7;
                foreach($this->super_model->custom_query("SELECT * FROM merge_collection_head ch INNER JOIN merge_collection_details cd ON ch.merge_collection_id = cd.merge_collection_id WHERE $qu GROUP BY series_number ORDER BY series_number ASC") AS $col){

                    $tin = $this->super_model->select_column_where("participant","tin","settlement_id",$col->settlement_id);
                    $address = $this->super_model->select_column_where("participant","registered_address","settlement_id",$col->settlement_id);
                    $zip = $this->super_model->select_column_where("participant","zip_code","settlement_id",$col->settlement_id);

                    $sum_amount= $this->super_model->select_sum_where("merge_collection_details","amount","merge_collection_id = '$col->merge_collection_id' AND series_number = '$col->series_number'");
                    $sum_zero_rated= $this->super_model->select_sum_where("merge_collection_details","zero_rated","merge_collection_id = '$col->merge_collection_id' AND series_number = '$col->series_number'");
                    $sum_zero_rated_ecozone= $this->super_model->select_sum_where("merge_collection_details","zero_rated_ecozone","merge_collection_id = '$col->merge_collection_id' AND series_number = '$col->series_number'");
                    $sum_vat = $this->super_model->select_sum_where("merge_collection_details","vat","merge_collection_id = '$col->merge_collection_id' AND series_number = '$col->series_number'");
                    $sum_ewt= $this->super_model->select_sum_where("merge_collection_details","ewt","merge_collection_id = '$col->merge_collection_id' AND series_number = '$col->series_number'");
                    $overall_total=($sum_amount + $sum_zero_rated + $sum_zero_rated_ecozone + $sum_vat)-$sum_ewt;
        ...
        header('Content-Type: application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
        header('Content-Disposition: attachment;filename="IEMOP Collection Reports (Merge).xlsx"');
        header('Cache-Control: max-age=0');
        $objWriter = io_factory::createWriter($objPHPExcel, 'Xlsx');
        $objWriter->save('php://output');
