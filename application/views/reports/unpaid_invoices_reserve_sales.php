                ...
                <div class="col-12">
                    <div class="card">
                        <div class="card-header">                        
                            <div class="row">
                                <div class="col-4">
                                    <h4>Summary of All Unpaid Invoices - Reserve Main</h4>
                                </div>
                                <div class="col-8">
                                    <!-- <button class="btn btn-primary btn-sm pull-right"><span class="fas fa-print"></span> Print</button> -->
                                    <?php if(!empty($unpaid_sales)){ ?>
                                        <a href = "<?php echo base_url();?>reports/export_unpaid_invoices_reserve_sales/<?php echo $year; ?>/<?php echo $due; ?>/" class = "btn btn-success pull-right">Export to Excel</a>
                                    <?php }else{ ?>
                                        <a href = "<?php echo base_url();?>/reports/export_unpaid_invoices_reserve_sales/" class = "btn btn-success pull-right">Export to Excel</a>
                                    <?php } ?>  
                                </div>
                            </div>
                        </div>
                                            ...
                                            <td width="20%">
                                                <select class="form-control select2" name="due_date" id="due_date">
                                                    ...
                                                    <?php foreach($due_date AS $dd){ ?>
                                                        <option value="<?php echo $dd->res_due_date;?>"><?php echo $dd->res_due_date;?></option>
                                                    <?php } ?>
                                                </select>
                                            </td>
                                                <td width="10%">
                                                    <input type="hidden" id="baseurl" value="<?php echo base_url();?>">
                                                    <button type="button" onclick="filterUnpaidReserveSales();" class="btn btn-primary btn-block">Filter</button>
                                                </td>
                                            </tr>
                                        </table>
                                    </form>
